...
export const initPeer = (userId) => {
  ...
  peer.on('error', (err) => {
    ...
    if (err.type === 'unavailable-id') {
      console.log('ID taken, retrying with suffix...');
      // If ID is taken, we could retry with a suffix, but usually it means 
      // an old connection is still hanging. PeerJS will eventually clean it up.
    }
  });
